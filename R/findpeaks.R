...
findpeaks <- function (data, MinPeakHeight = .Machine$double.eps, MinPeakDistance = 1,
  ...
  if (!isPosscal(MinPeakHeight)) stop ("MinPeakHeight must be a positive scalar")
  if (!isPosscal(MinPeakDistance)) stop ("MinPeakDistance must be a positive scalar")
  if (!isPosscal(MinPeakWidth)) stop ("MinPeakWidth must be a positive scalar")
  if (!is.logical(DoubleSided)) stop ("DoubleSided should a a logical value TRUE or FALSE")
  ...
  if (any(D) < MinPeakDistance) {
    ...
    while (length(node2visit) > 0) {
      ...
      if (length(neighs) > 0) {
        idx.pruned <- setdiff (idx.pruned, idx_s[neighs])
  ...
  for (i in 1:n) {
    ...
    if ((width > MaxPeakWidth || width < MinPeakWidth) || pp[1] > 0 || H < MinPeakHeight 
      ...
      extra.pp <- rbind(extra.pp, pp)
      extra.roots <- rbind(extra.roots, xm + c(-width, width) / 2)
      extra.height <- rbind(extra.height, H)
      extra.baseline <- rbind(extra.baseline, mean(c(H, MinPeakHeight)))
    }
  }
  idx <- idx.pruned
  ...
  list(pks = pks, loc = idx, parabol = list(x = as.list(extra.x), pp = as.list(extra.pp)),
       height = as.numeric(extra.height[, 1]), baseline = as.numeric(extra.baseline[, 1]),
       roots = as.list(extra.roots))
}
