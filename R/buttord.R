...
#'
#' For Low pass filters, \eqn{|H(W)|^{2} = 1/[1+(W/Wc)^{(2N)}] = 10^{(-R/10)}}.
...
#' one can recalculate the allowable range for \code{Wc} (filter caracteristic
...
#' freqz(but, fs = fs)
#'
#' @author Original Octave code by Paul Kienzle; adapted by Charles Praplan.
...
buttord <- function (Wp, Ws, Rp, Rs, plane = c("z", "s")) {
  ...
  IIRfspec(n = n, Wc = Wc_p, type = type, Wc_s = Wc_s, plane = plane)
}
