...
For causal discrete-time FIR filters the output is a weighted sum of the most recent input values. Compared to IIR filters, advantages of FIR filters are they are inherently stable (because there is no feedback that propagates indefinitely), and that they have linear phase (constant across frequencies). The main disadvantages is that they require more computation time to obtain sharp transition bands.
...
ll <- microbenchmark(conv(long, long),
                     cconv(long, long),
                     fftconv(long, long))
plot1 <- ggplot2::autoplot(ll)

# convolve a short and a long series
sl <- microbenchmark(conv(short, long),
                     cconv(short, long),
                     fftconv(short, long))
...
If your signal's frequency content changes over time, the power spectrum is of limited use. You might then want to use some form of time-frequency analysis. Specializes R packages exist for wavelet analysis capable of doing time-frequency analysis, such as [wavelets](https://cran.r-project.org/web/packages/wavelets/index.html), [Rwave](https://cran.r-project.org/web/packages/Rwave/), and [waveslim](https://cran.r-project.org/web/packages/waveslim/index.html). `gsignal` does contain a basic function for computing the discrete wavelet transform (`dwt`), but is not otherwise specialized for wavelet analysis.
...
* Windowing functions. [Window functions](https://en.wikipedia.org/wiki/Window_function) are usually bell-shaped functions (although rectangular and triangular shapes are also used), which are multiplied by the signal in order to taper the ends of the signal to zero to reduce spectral leakage. For instance, the `pwelch` function described above uses a window function (default Hamming) before FFT-ing the segments. The window functions available in `gsignal` are:
  ...
  + Bartlett (triangular - `bartlett`, alias `triang`)
  + Blackman window (`blackman`)
  + Blackman-Harris (`blackmanharris`)
  + Blackman-Nuttall (`blackmannuttall`)
  + Bohman (`bohmanwin`)
  + Boxcar (rectangular - `boxcar`, alias `rectwin`)
  + Chebyshev (`chebwin`)
  + Flat top (`flattopwin`)
  + Gaussian (`gausswin`)
  + Hamming (`hamming`),
